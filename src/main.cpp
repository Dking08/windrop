...
// ── Shared state between LL keyboard hook and QueryContinueDrag ──
...
static LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
        ...
        (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN))
    {
        ...
        {
            if (kb->vkCode == VK_F8)
            { g_shouldDrop = true; return 1; }
            if (kb->vkCode == VK_ESCAPE)
            { g_shouldCancel = true; return 1; }
        }
    }
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    {
        // Called via PostMessage after F8. We are now inside DispatchMessage
        // on our thread, which is the correct context for DoDragDrop.
        g_isDragging = true;
        DWORD dwOK = DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK;
        g_dragResult = DoDragDrop(g_pDataObj, g_pDropSrc, dwOK, &g_dwEffect);
        g_isDragging = false;
        PostQuitMessage(0); // break out of the message loop
        return 0;
    }
...
static HWND CreateHelperWindow(HINSTANCE hInst)
{
    ...
    // Real top-level popup — NOT message-only. Layered so it's invisible.
    ...
    if (hwnd)
        SetLayeredWindowAttributes(hwnd, 0, 1, LWA_ALPHA); // alpha=1, invisible
    return hwnd;
}
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    int exitCode = EXIT_COM_ERROR;
    {
        ...
        // DispatchMessage on our thread — this is the proper context.
        PostMessageW(hwnd, WM_START_DRAG, 0, 0);

        // Run message loop — DoDragDrop blocks inside the WM_START_DRAG
        // handler, but our LL hook callbacks still fire because DoDragDrop
        // runs its own nested message loop internally.
        while (GetMessageW(&msg, nullptr, 0, 0))
        { TranslateMessage(&msg); DispatchMessageW(&msg); }
